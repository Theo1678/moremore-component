...
const W1 = {
  ...
  setup(i, { expose: e, emit: t }) {
    const n = i, w = t, N = b(null), H = b(null), A = b(null), T = b(null), v = b(n.initialSlide), E = b(!1), p = b(!1), k = b({ x: 0, y: 0 }), r = b({ x: 0, y: 0 }), c = b(0), h = b(null), _ = x(() => n.loop ? n.items.length - 1 : Math.max(0, n.items.length - n.slidesPerView)), f = x(() => Math.floor(v.value / n.slidesPerView)), m = x(() => {
      const a = Math.ceil(n.items.length / n.slidesPerView);
      return Array.from({ length: a }, (g, I) => I);
      ...
      const a = -(v.value * (O.value + n.spaceBetween)) + c.value;
      return {
        display: "flex",
      ...
      const a = v.value / _.value, g = n.slidesPerView / n.items.length * 100, I = a * (100 - g);
      return {
        width: `${g}%`,
        left: `${I}%`,
    ...
    })), D1 = (a, g) => n.keyName && a[n.keyName] ? a[n.keyName] : g, R1 = (a) => ({}), G1 = (a) => ({
      width: `${O.value}px`,
      flexShrink: 0
    }), F = (a, g = n.speed) => {
      if (E.value && !p.value) return;
      const I = Math.max(0, Math.min(a, _.value));
      ...
      n.allowTouchMove && (p.value = !0, k.value = { x: a.clientX, y: a.clientY }, r.value = { x: a.clientX, y: a.clientY }, document.addEventListener("mousemove", z), document.addEventListener("mouseup", X), w("touchStart", a));
    }, z = (a) => {
      if (!p.value) return;
      r.value = { x: a.clientX, y: a.clientY };
      const g = (r.value.x - k.value.x) * n.touchRatio;
      c.value = g, w("touchMove", a);
    }, X = (a) => {
      if (!p.value) return;
      const g = r.value.x - k.value.x;
      Math.abs(g) > n.threshold && (g > 0 ? R() : D()), p.value = !1, c.value = 0, document.removeEventListener("mousemove", z), document.removeEventListener("mouseup", X), w("touchEnd", a);
    }, j1 = (a) => {
      if (!n.allowTouchMove) return;
      p.value = !0;
      const g = a.touches[0];
      k.value = { x: g.clientX, y: g.clientY }, r.value = { x: g.clientX, y: g.clientY }, w("touchStart", a);
    }, O1 = (a) => {
      if (!p.value) return;
      const g = a.touches[0];
      r.value = { x: g.clientX, y: g.clientY };
      const I = (r.value.x - k.value.x) * n.touchRatio;
      c.value = I, w("touchMove", a);
    }, z1 = (a) => {
      if (!p.value) return;
      const g = r.value.x - k.value.x;
      Math.abs(g) > n.threshold && (g > 0 ? R() : D()), p.value = !1, c.value = 0, w("touchEnd", a);
    }, J = () => {
    ...
    return X1(() => {
      ...
      a.showPagination ? (l(), s("div", {
        ...
        (l(!0), s(M, null, V(m.value, (I, Z) => (l(), s("button", {
          key: Z,
          onClick: (T9) => F(Z * a.slidesPerView),
  ...
  setup(i, { emit: e }) {
    const t = i, n = e, w = b(t.modelValue), N = (p) => {
      ...
      const k = w.value === p.id, r = "flex items-center justify-center focus:outline-none hover:border-[#F66D96] border-1 rounded-xl";
      return k ? `${r} bg-[#FFE8F0] border-[#F66D96] border-2` : `${r} bg-white border-[#CFD3DA] border-2`;
    }, A = (p) => {
      const k = w.value === p.id, c = `${t.isMobile ? "text-xs" : "text-sm"} font-bold text-center whitespace-nowrap`;
      return k ? `${c} text-[#F66D96]` : `${c} text-[#424855]`;
    }, T = x(() => {
      if (t.tabs.length <= 10) {
        const r = [];
        for (let c = 0; c < t.tabs.length; c += 5)
          r.push(t.tabs.slice(c, c + 5));
        return r;
      } else {
        const r = [];
        for (let c = 0; c < t.tabs.length; c += 8)
          r.push(t.tabs.slice(c, c + 8));
        return r;
      }
