import { defineComponent as c, createElementBlock as o, openBlock as l, createElementVNode as t, ref as $, computed as I, onMounted as j1, onUnmounted as D1, watch as J, createBlock as d, createCommentVNode as k, normalizeClass as M, normalizeStyle as X, Fragment as F, renderList as T, renderSlot as G, createTextVNode as E, toDisplayString as b, createVNode as X1, withCtx as z1, withModifiers as Q, unref as C } from "vue";
const U1 = {
  width: "24",
  height: "25",
  viewBox: "0 0 24 25",
  ...
  setup(i, { expose: e, emit: n }) {
    const s = i, v = n, S = $(null), B = $(null), A = $(null), L = $(null), u = $(s.initialSlide), m = $(!1), x = $(!1), V = $({ x: 0, y: 0 }), f = $({ x: 0, y: 0 }), _ = $(0), p = $(null), a = I(() => s.loop ? s.items.length - 1 : Math.max(0, s.items.length - s.slidesPerView)), w = I(() => Math.floor(u.value / s.slidesPerView)), g = I(() => {
      const r = Math.ceil(s.items.length / s.slidesPerView);
      return Array.from({ length: r }, (h, y) => y);
    }), Z = I(() => (console.log("showPrevButton", s.loop, u.value, a.value), s.loop || u.value > 0)), O = I(() => (console.log("showNextButton", s.loop, u.value, a.value), s.loop || u.value < a.value)), W = I(() => B.value ? (B.value.clientWidth - (s.slidesPerView - 1) * s.spaceBetween) / s.slidesPerView : 0), H1 = I(() => ({
      overflow: "hidden",
      position: "relative"
    })), B1 = I(() => {
      const r = -(u.value * (W.value + s.spaceBetween)) + _.value;
      return {
        display: "flex",
        transition: m.value && !x.value ? `transform ${s.speed}ms ${s.easing}` : "none",
        transform: `translateX(${r}px)`,
        gap: `${s.spaceBetween}px`
      };
    }), V1 = I(() => {
      if (!s.items.length) return { width: "0%", left: "0%" };
      const r = u.value / a.value, h = s.slidesPerView / s.items.length * 100, y = r * (100 - h);
      return {
        width: `${h}%`,
        left: `${y}%`,
    ...
    })), S1 = (r, h) => s.keyName && r[s.keyName] ? r[s.keyName] : h, A1 = (r) => ({}), P1 = (r) => ({
      width: `${W.value}px`,
      flexShrink: 0
    }), P = (r, h = s.speed) => {
      if (m.value && !x.value) return;
      const y = Math.max(0, Math.min(r, a.value));
      y !== u.value && (m.value = !0, v("transitionStart", { index: y }), u.value = y, setTimeout(() => {
        m.value = !1, v("transitionEnd", { index: y }), v("slideChange", { index: y, item: s.items[y] }), y === 0 && v("reachBeginning"), y === a.value && v("reachEnd");
      }, h));
    }, N = () => {
      s.loop && u.value === a.value ? P(0) : P(u.value + 1);
    }, D = () => {
      s.loop && u.value === 0 ? P(a.value) : P(u.value - 1);
    }, E1 = (r) => {
      s.allowTouchMove && (x.value = !0, V.value = { x: r.clientX, y: r.clientY }, f.value = { x: r.clientX, y: r.clientY }, document.addEventListener("mousemove", z), document.addEventListener("mouseup", U), v("touchStart", r));
    }, z = (r) => {
      if (!x.value) return;
      f.value = { x: r.clientX, y: r.clientY };
      const h = (f.value.x - V.value.x) * s.touchRatio;
      _.value = h, v("touchMove", r);
    }, U = (r) => {
      if (!x.value) return;
      const h = f.value.x - V.value.x;
      Math.abs(h) > s.threshold && (h > 0 ? D() : N()), x.value = !1, _.value = 0, document.removeEventListener("mousemove", z), document.removeEventListener("mouseup", U), v("touchEnd", r);
    }, F1 = (r) => {
      if (!s.allowTouchMove) return;
      x.value = !0;
      const h = r.touches[0];
      V.value = { x: h.clientX, y: h.clientY }, f.value = { x: h.clientX, y: h.clientY }, v("touchStart", r);
    }, T1 = (r) => {
      if (!x.value) return;
      const h = r.touches[0];
      f.value = { x: h.clientX, y: h.clientY };
      const y = (f.value.x - V.value.x) * s.touchRatio;
      _.value = y, v("touchMove", r);
    }, N1 = (r) => {
      if (!x.value) return;
      const h = f.value.x - V.value.x;
      Math.abs(h) > s.threshold && (h > 0 ? D() : N()), x.value = !1, _.value = 0, v("touchEnd", r);
    }, Y = () => {
      if (!s.autoplay) return;
      const r = typeof s.autoplay == "object" && s.autoplay.delay || 3e3;
      p.value = setInterval(() => {
        N();
      }, r);
    }, q = () => {
    ...
    return j1(() => {
      ...
      q(), document.removeEventListener("mousemove", z), document.removeEventListener("mouseup", U);
    }), J(
      () => s.autoplay,
      (r) => {
        r ? Y() : q();
      ...
      t("div", {
        ...
        t("div", {
          ...
          (l(!0), o(F, null, T(r.items, (y, H) => (l(), o("div", {
            key: S1(y, H),
            class: M(["swiper-slide", {
              "swiper-slide-active": H === u.value,
              "swiper-slide-prev": H === u.value - 1,
              "swiper-slide-next": H === u.value + 1,
              ...A1()
            }]),
            style: X(P1())
          }, [
      ...
      r.showNavigation ? (l(), d(R, {
        key: 1,
        onClick: N,
        class: M(["swiper-button-next", {
          "type-multiple": r.multiple,
          "swiper-button-disabled": u.value === a.value,
          "swiper-button-hidden": !O.value
        }])
      }, null, 8, ["class"])) : k("", !0),
      r.showPagination ? (l(), o("div", {
        key: 2,
        class: M(["swiper-pagination", Z1.value])
      }, [
        (l(!0), o(F, null, T(g.value, (y, H) => (l(), o("button", {
          key: H,
          onClick: (n9) => P(H * r.slidesPerView),
          class: M(["swiper-pagination-bullet", {
            "swiper-pagination-bullet-active": H === w.value
          }])
        }, [
          G(r.$slots, "pagination-bullet", {
            index: H,
            isActive: H === w.value
          }, () => [
            h[0] || (h[0] = t("span", { class: "swiper-bullet-dot" }, null, -1))
          ], !0)
        ], 10, Y1))), 128))
      ], 2)) : k("", !0),
      r.showScrollbar ? (l(), o("div", {
        key: 3,
  ...
  for (const [s, v] of e)
    n[s] = v;
  return n;
}, l1 = /* @__PURE__ */ j(q1, [["__scopeId", "data-v-6ed75995"]]), G1 = { class: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 !gap-8 mb-12 items-start" }, K1 = { class: "w-full max-w-2xl mx-auto bg-white rounded-2xl border border-gray-200 overflow-hidden h-full flex flex-col" }, J1 = ["onClick"], Q1 = { class: "flex items-center gap-4 h-full" }, e2 = { class: "w-24 h-24 rounded-full border border-gray-300 bg-gray-50 flex-shrink-0 overflow-hidden" }, l2 = ["src", "alt", "onError"], t2 = {
  key: 1,
  class: "w-full h-full flex flex-col items-center justify-center gap-1"
}, o2 = { class: "flex-1 min-w-0 text-left flex flex-col justify-center" }, n2 = { class: "flex items-center justify-between gap-3 mb-2" }, s2 = { class: "font-pretendard text-[17px] font-bold leading-[22px] line-clamp-2 text-[#303040]" }, r2 = { class: "text-xs font-medium text-[#303040]" }, i2 = {
  key: 0,
  ...
  setup(i, { emit: e }) {
    const n = i, s = e, v = $(/* @__PURE__ */ new Set()), S = $(n.maxItems), B = I(() => n.shops.slice(0, S.value)), A = (f) => {
      s("shop-click", f);
    }, L = (f) => {
      s("participant-click", f);
    }, u = (f) => {
      const _ = "flex items-center gap-2 px-2 py-1 rounded-md";
      switch (f) {
        case "blue":
          return `${_}`;
        case "green":
      ...
      switch (f) {
        case "yellow":
          return `${_} bg-yellow-400`;
        case "gray":
        default:
          return `${_} bg-gray-400`;
      }
    }, x = (f) => {
      v.value.add(f);
    }, V = (f) => f.image && !v.value.has(f.id);
    return (f, _) => (l(), o("div", G1, [
      (l(!0), o(F, null, T(B.value, (p) => (l(), o("div", {
        key: p.id,
            ...
            t("div", Q1, [
              ...
              t("div", o2, [
                t("div", n2, [
                  ...
                  p.badge ? (l(), o("div", {
                    key: 0,
                    class: M([u(p.badgeColor), "flex-shrink-0"])
                  }, [
                    t("div", {
                      class: M(m(p.badgeColor))
                    }, null, 2),
                    t("span", r2, b(p.badge), 1)
                  ], 2)) : k("", !0)
                ]),
                p.description ? (l(), o("p", i2, b(p.description), 1)) : k("", !0),
                p.period ? (l(), o("p", a2, b(p.period), 1)) : k("", !0)
              ])
            ])
          ], 8, J1),
          _[3] || (_[3] = t("div", { class: "border-t border-gray-200" }, null, -1)),
          t("div", c2, [
            _[2] || (_[2] = t("h3", { class: "text-left font-pretendard text-[15px] font-bold leading-[22px] mb-2 text-[#303040]" }, " 참여작가 ", -1)),
            p.participants && p.participants.length > 0 ? (l(), o("div", {
              key: 0,
...
}), _2 = { class: "w-full" }, v2 = ["onClick"], w2 = { class: "relative aspect-[4/3] rounded-lg border border-gray-200" }, m2 = ["src", "alt", "onError"], g2 = {
  key: 1,
  class: "w-full h-full flex flex-col items-center justify-center gap-2"
}, y2 = { class: "p-3 text-left" }, $2 = {
  key: 0,
  class: "mb-2"
}, L2 = { class: "text-sm font-bold text-[#060608] mb-2 line-clamp-1" }, x2 = {
  key: 1,
  ...
  key: 1,
  class: "w-full h-full flex items-center justify-center"
}, Z2 = { class: "text-xs text-[#060608] truncate" }, S2 = /* @__PURE__ */ c({
  ...
  emits: ["shop-click", "participant-click"],
  setup(i, { emit: e }) {
    const n = i, s = e, v = $(!1), S = $(null), B = $(/* @__PURE__ */ new Set()), A = $(n.maxItems), L = I(() => n.shops.slice(0, A.value)), u = (a) => {
      S.value = a, v.value = !0, s("shop-click", a);
    }, m = (a) => {
      ...
      switch (a) {
        case "blue":
          return `${w} bg-blue-100`;
        case "yellow":
          return `${w} bg-yellow-100`;
        case "gray":
        default:
          return `${w} bg-gray-100`;
      }
    }, V = (a) => {
      const w = "w-1.5 h-1.5 rounded-full";
      switch (a) {
        case "blue":
          return `${w} bg-blue-400`;
        case "yellow":
      ...
      B.value.add(a);
    }, _ = (a) => a.image && !B.value.has(a.id), p = I(() => {
      const a = n.itemsPerRow;
      let w = "grid grid-cols-1";
      return a >= 2 && (w += " md:grid-cols-2"), a >= 3 && (w += " lg:grid-cols-3"), a >= 4 && (w += " xl:grid-cols-4"), a >= 5 && (w += " 2xl:grid-cols-5"), a >= 6 && (w += " 2xl:grid-cols-6"), `${w} gap-8`;
    });
    return (a, w) => (l(), o("div", _2, [
      t("div", {
        class: M(p.value)
      }, [
        (l(!0), o(F, null, T(L.value, (g) => (l(), o("div", {
          key: g.id,
          ...
          t("div", y2, [
            g.badge ? (l(), o("div", $2, [
              t("div", {
                class: M([x(g.badgeColor), "text-[#303040]"])
              }, [
                t("div", {
            ...
            t("h3", L2, b(g.title), 1),
            g.description ? (l(), o("p", x2, b(g.description), 1)) : k("", !0),
            g.period ? (l(), o("p", I2, b(g.period), 1)) : k("", !0),
            g.hashtags ? (l(), o("p", k2, b(g.hashtags), 1)) : k("", !0),
            g.participants && g.participants.length > 0 ? (l(), o("div", b2, [
              (l(!0), o(F, null, T(g.participants, (Z) => (l(), o("div", {
                key: Z.id,
...
}), A2 = /* @__PURE__ */ j(S2, [["__scopeId", "data-v-d3412538"]]), P2 = { class: "w-full" }, E2 = { class: "flex items-stretch gap-4 px-5 flex-wrap" }, F2 = ["onClick"], T2 = {
  __name: "TabGroup",
  props: {
    tabs: {
      type: Array,
    ...
    variant: {
      ...
      default: "default",
      // 'default' | 'compact'
      validator: (i) => ["default", "compact"].includes(i)
    }
  },
  emits: ["update:modelValue", "tab-change"],
  setup(i, { emit: e }) {
    const n = i, s = e, v = $(n.modelValue), S = (L) => {
      v.value = L.id, s("update:modelValue", L.id), s("tab-change", L);
    }, B = (L) => {
      const u = v.value === L.id, m = "flex items-center justify-center focus:outline-none hover:border-[#F66D96] border-2";
      return u ? `${m} bg-[#FFE8F0] border-[#F66D96]` : `${m}`;
    }, A = (L) => {
      const u = v.value === L.id, m = "text-sm font-bold text-center whitespace-nowrap";
      return u ? `${m} text-[#F66D96]` : `${m} text-gray-700`;
    };
    return J(
      () => n.modelValue,
      (L) => {
      ...
      t("div", E2, [
        (l(!0), o(F, null, T(i.tabs, (m) => (l(), o("button", {
          key: m.id,
          class: M(B(m)),
          onClick: (x) => S(m),
          type: "button"
...
}, N2 = /* @__PURE__ */ j(T2, [["__scopeId", "data-v-1ce4eda4"]]), j2 = /* @__PURE__ */ c({
  __name: "ElementIcon",
  props: {
    elementIcon: {},
    color: {}
  ...
  setup(i) {
    return (e, n) => e.elementIcon === "IcApp" ? (l(), d(C(t1), {
      key: 0,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcChart" ? (l(), d(C(o1), {
      key: 1,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcPerson" ? (l(), d(C(c1), {
      key: 2,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcOption" ? (l(), d(C(i1), {
      key: 3,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcHelp" ? (l(), d(C(n1), {
      key: 4,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcInfo" ? (l(), d(C(s1), {
      key: 5,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcNotification" ? (l(), d(C(r1), {
      key: 6,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcPayback" ? (l(), d(C(a1), {
      key: 7,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcPlane" ? (l(), d(C(d1), {
      key: 8,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcSetting" ? (l(), d(C(C1), {
      key: 9,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcUpdate" ? (l(), d(C(u1), {
      key: 10,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcMedia" ? (l(), d(C(p1), {
      key: 11,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcLink" ? (l(), d(C(h1), {
      key: 12,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcArrowRight" ? (l(), d(C(R), {
      key: 13,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcDown" ? (l(), d(C(f1), {
      key: 14,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcCopy" ? (l(), d(C(_1), {
      key: 15,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcDelete" ? (l(), d(C(v1), {
      key: 16,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcPlus" ? (l(), d(C(w1), {
      key: 17,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcMoreVertical" ? (l(), d(C(m1), {
      key: 18,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcCancleFill" ? (l(), d(C(g1), {
      key: 19,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcChecked" ? (l(), d(C(y1), {
      key: 20,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcSparkle" ? (l(), d(C($1), {
      key: 21,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcPreview" ? (l(), d(C(L1), {
      key: 22,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcLoadingPrimary" ? (l(), d(C(k1), {
      key: 23,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcLayer" ? (l(), d(C(b1), {
      key: 24,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcCta" ? (l(), d(C(M1), {
      key: 25,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcSearch" ? (l(), d(C(x1), {
      key: 26,
      color: e.color
    }, null, 8, ["color"])) : e.elementIcon === "IcClose" ? (l(), d(C(I1), {
      key: 27,
      color: e.color
    }, null, 8, ["color"])) : k("", !0);
  }
}), D2 = /* @__PURE__ */ j(j2, [["__scopeId", "data-v-c7e0d0ce"]]), X2 = {
  width: "24",
  height: "24",
  viewBox: "0 0 24 24",
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, z2 = ["fill"], t1 = /* @__PURE__ */ c({
  __name: "IcApp",
  props: {
    color: { default: () => "#ABAFB9" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", X2, [
      t("path", {
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, R2 = ["fill"], o1 = /* @__PURE__ */ c({
  __name: "IcChart",
  props: {
    color: { default: () => "#ABAFB9" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", U2, [
      t("path", {
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, o3 = ["fill"], c1 = /* @__PURE__ */ c({
  __name: "IcPerson",
  props: {
    color: { default: () => "#ABAFB9" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", t3, [
      t("path", {
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, v3 = { id: "Group 395" }, w3 = ["fill"], m3 = ["fill"], _1 = /* @__PURE__ */ c({
  __name: "IcCopy",
  props: {
    color: { default: () => "#5C6476" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", _3, [
      t("g", v3, [
        t("path", {
          id: "Vector (Stroke)",
          "fill-rule": "evenodd",
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, L3 = { id: "ic-plus" }, x3 = ["fill"], w1 = /* @__PURE__ */ c({
  __name: "IcPlus",
  props: {
    color: { default: () => "#5C6476" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", $3, [
      t("g", L3, [
        t("path", {
          id: "Union",
          "fill-rule": "evenodd",
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, k3 = ["fill"], b3 = ["fill"], M3 = ["fill"], m1 = /* @__PURE__ */ c({
  __name: "IcMoreVertical",
  props: {
    color: { default: () => "#5C6476" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", I3, [
      t("ellipse", {
        cx: "10",
        cy: "15.8333",
        rx: "1.25",
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, E3 = ["fill"], L1 = /* @__PURE__ */ c({
  __name: "IcPreview",
  props: {
    color: { default: () => "#6540DE" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", P3, [
      t("path", {
        "fill-rule": "evenodd",
        "clip-rule": "evenodd",
  ...
  viewBox: "0 0 21 21",
  fill: "none"
}, T3 = ["fill"], x1 = /* @__PURE__ */ c({
  __name: "IcSearch",
  props: {
    color: { default: () => "#5C6476" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", F3, [
      t("path", {
        "fill-rule": "evenodd",
        "clip-rule": "evenodd",
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, z3 = { "clip-path": "url(#clip0_3951_314363)" }, U3 = ["fill"], R3 = ["fill"], O3 = ["fill"], W3 = ["fill"], Y3 = ["fill"], q3 = ["fill"], G3 = ["fill"], K3 = ["fill"], J3 = /* @__PURE__ */ c({
  __name: "IcLoadingPrimary",
  props: {
    color: { default: () => "#6540DE" }
  },
  setup(i) {
    return (e, n) => (l(), o("div", D3, [
      (l(), o("svg", X3, [
        t("g", z3, [
          t("path", {
        ...
        n[0] || (n[0] = t("defs", null, [
          t("clipPath", { id: "clip0_3951_314363" }, [
            t("rect", {
              width: "20",
              height: "20",
              fill: "white"
...
}), k1 = /* @__PURE__ */ j(J3, [["__scopeId", "data-v-9156a366"]]), Q3 = {
  width: "20",
  height: "20",
  viewBox: "0 0 20 20",
  fill: "none",
  ...
  fill: "none",
  xmlns: "http://www.w3.org/2000/svg"
}, t9 = ["fill"], M1 = /* @__PURE__ */ c({
  __name: "IcCta",
  props: {
    color: { default: () => "#716497" }
  },
  setup(i) {
    return (e, n) => (l(), o("svg", l9, [
      t("path", {
        "fill-rule": "evenodd",
        "clip-rule": "evenodd",
  ...
  install(i, e = {}) {
    K.forEach((n) => {
      ...
      const s = n.__name || n.name || "UnnamedComponent";
      i.component(`${e.prefix}${s}`, n);
    });
  }
};
typeof window < "u" && window.Vue && window.Vue.use(o9);
export {
